...
export async function saveHomeAddress(userId, addressText, lat, lng, geocodeStatus) {
  ...
  if (error) {
    console.error('[addressCache] saveHomeAddress error:', error, 'payload:', row)
    const detail = error.message || error.details || error.hint || error.code || 'unknown error'
    throw new Error('Failed to save geocoded home address: ' + detail)
  }

  if (!data) {
    // Upsert succeeded but row wasn't returned (RLS hides it from the read-back
    // because auth.uid() does not match the user_id we wrote, or the session is
    // missing). Fall back to a fresh fetch — that read uses the same RLS, so if
    // it also returns nothing, the auth state is broken.
    const fallback = await getHomeAddress(userId)
    if (!fallback) {
      throw new Error(
        'Failed to save geocoded home address: row not visible after upsert ' +
        '(check that you are signed in).'
      )
    }
    return { data: fallback, addressChanged, newVersion }
  }
...
export async function saveConfirmedDistance(
  ...
  if (error) {
    console.error('[addressCache] saveConfirmedDistance error:', error, 'payload:', row)
    const detail = error.message || error.details || error.hint || error.code || 'unknown error'
    throw new Error('Failed to save confirmed station distance: ' + detail)
  }
}
