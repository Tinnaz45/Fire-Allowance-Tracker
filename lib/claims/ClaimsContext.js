...
import { CLAIM_TABLES } from '@/lib/claims/claimTypes'
...
async function fetchClaimsForFY(userId, financialYearId, fyStartDate, fyEndDate) {
  ...
  for (let i = 0; i < CLAIM_TABLES.length; i++) {
    ...
    if (data) {
      data.forEach((row) => {
        combined.push({ ...row, claimType: CLAIM_TABLES[i] })
      })
    }
  }
...
export function ClaimsProvider({ children }) {
  ...
  const addClaim = useCallback(async ({ userId, claimType, date, breakdown, fields, rates, financialYearId }) => {
    ...
    try {
      ...
      const row = builder(userId, date, breakdown, fields, ratesSnapshot, financialYearId, claimNumber, claimGroupId)
      const { error: insertError } = await supabase.from(claimType).insert(row)
  ...
  const updateClaim = useCallback(async ({ userId, claim, date, amount, status, financialYearId }) => {
    ...
    try {
      ...
      const amountColumn = claim.claimType === 'spoilt' ? 'meal_amount' : 'total_amount'
  ...
  //   { group, children: Claim[] }
  // Plus ungrouped claims (no claim_group_id) as flat items.

  const groupedView = (() => {
    const grouped = []
    const ungrouped = []
    const childIds = new Set(claims.filter((c) => c.claim_group_id).map((c) => c.id + c.claimType))
