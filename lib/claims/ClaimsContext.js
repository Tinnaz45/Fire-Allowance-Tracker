...
//   - claim_number is generated atomically via fat_increment_claim_sequence() RPC.
//   - Grouped claims: parent fat_claim_groups row + child rows with claim_group_id.
...
//   - fat_claim_groups are loaded separately and merged into grouped display.
//
...
// Calls the DB function fat_increment_claim_sequence() atomically.
...
async function getNextClaimNumber(userId, financialYearId, claimType) {
  const { data, error } = await supabase.rpc('fat_increment_claim_sequence', {
    p_user_id:           userId,
...
// ─── Claim group creator ──────────────────────────────────────────────────────
// Creates the parent fat_claim_groups row and returns it.
...
async function fetchClaimsForFY(userId, financialYearId, fyStartDate, fyEndDate) {
  ...
  for (let i = 0; i < CLAIM_TABLES.length; i++) {
    ...
    if (data) {
      ...
      data.forEach((row) => {
        // Spoilt table rows are split into two virtual claimTypes based on meal_type:
        //   meal_type 'Delayed' (or legacy 'Delayed') → claimType 'delayed_meal'
...
// Returns 'Paid' only if ALL children are Paid; otherwise 'Pending'.
// A single Disputed child → 'Disputed'.

function calcParentStatus(childClaims) {
  if (!childClaims || childClaims.length === 0) return 'Pending'
  const statuses = childClaims.map((c) => (c.status || 'Pending').toLowerCase())
...
export function ClaimsProvider({ children }) {
  ...
  //   2. Create fat_claim_groups parent row (label includes number + date)
  ...
  const addClaim = useCallback(async ({ userId, claimType, date, breakdown, fields, rates, financialYearId }) => {
    ...
    try {
      ...
      const { error: insertError } = await supabase.from(dbTable).insert(row)
      ...
      if (claimGroupId) {
        ...
        for (const { table, row: childRow } of childDefs) {
          ...
          const { error: childError } = await supabase.from(table).insert(insertRow)
          if (childError) {
  ...
  // Mirrors change to fat_payment_components ledger if a row exists.
  ...
  const updatePaymentStatus = useCallback(async ({ userId, claim, paymentStatus, financialYearId }) => {
    ...
    try {
      ...
      // 2. Mirror to fat_payment_components ledger if a row exists for this claim
      try {
        const { data: componentRows } = await supabase
          .from('fat_payment_components')
          .select('id')
  ...
  async function recomputeGroupStatus(groupId, userId, financialYearId) {
    // Fetch all children for this group across all tables
    const results = await Promise.all(
      CLAIM_TABLES.map((table) =>
        supabase
          .from(table)
  ...
  //   { group, children: Claim[], derivedPaymentStatus }
  // Plus ungrouped claims (no claim_group_id) as flat items.
  //
  // derivedPaymentStatus (Phase 2): computed client-side from child payment_status.
  //   'Paid'    = all children with payment_status set are Paid
  //   'Pending' = any child has payment_status = Pending (or NULL = legacy)
  //   'Legacy'  = no children have payment_status set at all
  ...
  const groupedView = (() => {
    ...
    // Attach children to groups + derive payment status
    for (const group of claimGroups) {
      const children = childrenByGroup[group.id] || []

      // Derive payment status from child payment_status fields (Phase 2)
      const childrenWithPaymentStatus = children.filter((c) => c.payment_status != null)
      let derivedPaymentStatus = 'Legacy'
      if (childrenWithPaymentStatus.length > 0) {
        const allPaid = childrenWithPaymentStatus.every(
          (c) => (c.payment_status || '').toLowerCase() === 'paid'
        )
        derivedPaymentStatus = allPaid ? 'Paid' : 'Pending'
      }

      grouped.push({ group, children, derivedPaymentStatus })
    }
