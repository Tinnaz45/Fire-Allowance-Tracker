...
// parent_status in fat_claim_groups is a CACHED PROJECTION, not truth.
...
export function ClaimsProvider({ children }) {
  ...
  // child rows, then writes the derived parent_status to fat_claim_groups.
  ...
  const groupedView = (() => {
    ...
    for (const group of claimGroups) {
      ...
      const derivedPaymentStatus = totalCount > 0 && paidCount === totalCount ? 'Paid' : 'Pending'
