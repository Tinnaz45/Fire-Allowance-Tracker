...
export function ClaimsProvider({ children }) {
  ...
  const addClaim = useCallback(async ({ userId, claimType, date, breakdown, fields, rates, financialYearId }) => {
    ...
    try {
      ...
      const { data: insertedRows, error: insertError } = await fat.from(dbTable).insert(row).select('id').limit(1)
      if (insertError) throw insertError
      const insertedClaimId = insertedRows && insertedRows[0] ? insertedRows[0].id : null
      ...
      // Return new claim metadata so callers (e.g. friend-replication picker)
      // can act on it without re-querying.
      return { claimId: insertedClaimId, claimTable: dbTable, claimType }
    } finally {
